...
from models.encoder.swin_transformer_encoder import RoPE_SwinViT_Encoder
...
class UniDepthExtended(nn.Module):
    def __init__(self, cfg):
        ...
        elif cfg.model.backbone.name == "rope_swinvit":
            self.encoder = RoPE_SwinViT_Encoder(
                model_name=cfg.model.backbone.model_name,
                pretrained=cfg.model.backbone.weights_init == "pretrained"
            )
            if cfg.model.backbone.depth_cond : 
                self.encoder.encoder.swin.embeddings.patch_embeddings.projection = nn.Conv2d(
                4, 
                self.encoder.encoder.swin.embeddings.patch_embeddings.projection.out_channels,
                kernel_size=self.encoder.encoder.swin.embeddings.patch_embeddings.projection.kernel_size,
                stride=self.encoder.encoder.swin.embeddings.patch_embeddings.projection.stride,
                padding=self.encoder.encoder.swin.embeddings.patch_embeddings.projection.padding,
                bias=self.encoder.encoder.swin.embeddings.patch_embeddings.projection.bias is not None
            )
